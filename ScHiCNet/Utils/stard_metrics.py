...
class VisionMetrics:
    ...
    def setDataset(self, chro="test", percent=0.75, cellN=21, cell_line="Dros"):
        if cell_line == "Dros":
        ...
        if cell_line == "Mouse":
            dm_test = GSE162511Module(batch_size=1, percent=percent, cell_No=cellN)
    ...
    def getMetrics(self, model, spliter):
        ...
        test_result = {'mse': 0, 'ssims': 0, 'psnr': 0, 'ssim': 0, 'nsamples': 0, 'pccs': 0, 'pcc': 0, 'spcs': 0,
                       'spc': 0, 'snrs': 0, 'snr': 0}
        ...
        with torch.no_grad():
            for lr, hr, inds in test_bar:
                ...
                # if spliter == "hicplus" or spliter == "hicsrNet":  # need padding the input data
                #     temp = F.pad(lr, (6, 6, 6, 6), mode='constant')
                #     out = model(temp)
                if spliter == "hicplus" or spliter == "hicsrNet":
                    temp = F.pad(lr, (6, 6, 6, 6), mode='constant')
                    out = model(temp)
                    out = out[..., 6:-6, 6:-6]
                # others model
                if spliter == "deephic" or spliter == 'unet' or spliter == 'hicarn' or spliter == "schicnet" :  # no need padding the data
                    out = model(lr)
