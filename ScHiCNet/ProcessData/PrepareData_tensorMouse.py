...
def splitPieces(fn, piece_size, step):
    ...
    for i in range(0, bound, step): # for the entire map
        for j in range(i, bound, step):
            if abs(i - j) <= 81 and i + step < bound and j + step < bound:
...
def loadBothConstraints(stria, strib, res, percent):
    ...
    print(np.percentile(mata, 99.9), np.percentile(mata, 99.99), per_a, np.max(mata))
    mata        = np.clip(mata, 0, per_a)
    mata        = mata/per_a

    #print(f'the whole mat shape:{matb.shape} and max number:{np.max(matb)}\n')
    #print(matb)
    matb_down = downsampling(matb, percent)
    matb = np.delete(matb, removeidx, axis=0)
    matb = np.delete(matb, removeidx, axis=1)
    print(f'After delete the matb shape:{matb.shape} and max number:{np.max(matb)}\n')
    #print(matb)

    gc.collect()
    per_b       = np.percentile(matb, 99.99)
    print(np.percentile(matb, 99.9), np.percentile(matb, 99.99), per_b, np.max(matb))
    ...
    print(np.percentile(matb_down, 99.9), np.percentile(matb_down, 99.99),  per_d, np.max(matb_down))
    matb_down = np.clip(matb_down, 0, per_d)
    matb_down = matb_down / per_d

    return matb, matb_down

class GSE162511Module(pl.LightningDataModule):
    def __init__(self,
                 batch_size = 64,
                 res = 40000,
                 percent = 0.75,
    ...
    def extract_constraint_mats(self):
        ...
        file_inter = glob.glob('../Datasets/Mouse/'+r'*cell'+str(self.cellNo)+r'*.mcool')
    ...
    def extract_create_numpy(self):
        ...
        for i in range(1, 20):
            ...
            np.save(self.dirname+"/Full_Mats/GSE162511_mat_full_chr_" + str(i) + "_" + str(self.res), target)
            np.save(self.dirname+"/Full_Mats/GSE162511_mat_" + str(self.percent) + "_chr_" + str(i) + "_" + str(self.res), data)

    def split_numpy(self):
        if not os.path.exists(self.dirname+"/Splits"):
            subprocess.run("mkdir -p "+self.dirname+"/Splits", shell = True)

        globs = glob.glob(self.dirname+"/Full_Mats/GSE162511_mat_full_chr_1_" + str(self.res) + ".npy")
        if len(globs) == 0:
            self.extract_create_numpy()

        for i in range(1, 20):
            target = splitPieces(self.dirname+"/Full_Mats/GSE162511_mat_full_chr_" + str(i) + "_" + str(self.res) + ".npy",
                                    self.piece_size, self.step)
            data = splitPieces(self.dirname+"/Full_Mats/GSE162511_mat_" + str(self.percent) + "_chr_" + str(i) + "_" + str(self.res) + ".npy",
                                  self.piece_size, self.step)

            np.save(
                self.dirname+"/Splits/GSE162511_full_chr_" + str(i) + "_" + str(self.res) + "_piece_" + str(self.piece_size),
                target)
            np.save(
                self.dirname+"/Splits/GSE162511_" + str(self.percent) + "_chr_" + str(i) + "_" + str(self.res) + "_piece_" + str(self.piece_size),
                data)

    def prepare_data(self):
        print("Preparing the Preparations ...")
        globs = glob.glob(
            self.dirname+"/Splits/GSE162511_full_chr_*_" + str(self.res) + "_piece_" + str(self.piece_size) + str(".npy"))
        if len(globs) > 20:
            print("Ready to go")
        else:
    ...
    class GSE162511Dataset(Dataset):
        def __init__(self, full, tvt, res, piece_size, percent, dir):
            ...
            if full == True:
                if tvt in list(range(1, 20)):
                    self.chros = [tvt]
                if tvt == "train":
                    self.chros = [1, 3, 5, 7, 8, 9, 11, 13, 15, 16, 17, 19]
                elif tvt == "val":
                    self.chros = [4, 14,18] #
                elif tvt == "test":
                ...
                self.target = np.load(
                    self.dir+"/Splits/GSE162511_full_chr_" + str(self.chros[0]) + "_" + str(self.res) + "_piece_" + str(
                        self.piece_size) + ".npy")
                self.data = np.load(
                    self.dir+"/Splits/GSE162511_" + str(self.percent) + "_chr_" + str(self.chros[0]) + "_" + str(self.res) + "_piece_" + str(
                        self.piece_size) + ".npy")
                self.info = np.repeat(self.chros[0], self.data.shape[0])
                for c, chro in enumerate(self.chros[1:]):
                    temp = np.load(
                        self.dir+"/Splits/GSE162511_full_chr_" + str(chro) + "_" + str(self.res) + "_piece_" + str(
                            self.piece_size) + ".npy")
                    ...
                    temp = np.load(
                        self.dir+"/Splits/GSE162511_" + str(self.percent) + "_chr_" + str(chro) + "_" + str(self.res) + "_piece_" + str(
                            self.piece_size) + ".npy")
                    if len(temp) == 0:
                        pass
                    else:
    ...
    def setup(self, stage = None):
        if stage in list(range(1, 20)):
            self.test_set = self.GSE162511Dataset(full = True, tvt = stage, res = self.res, piece_size = self.piece_size, percent = self.percent, dir = self.dirname)
        if stage == 'fit':
            self.train_set = self.GSE162511Dataset(full = True, tvt = 'train', res = self.res, piece_size = self.piece_size, percent = self.percent, dir = self.dirname)
            self.val_set = self.GSE162511Dataset(full = True, tvt = 'val', res = self.res, piece_size = self.piece_size, percent = self.percent, dir = self.dirname)
        if stage == 'test':
            self.test_set = self.GSE162511Dataset(full = True, tvt = 'test', res = self.res, piece_size = self.piece_size, percent = self.percent, dir = self.dirname)
...
if __name__ == '__main__':
    obj = GSE162511Module(percent = 0.75, cell_No = 1)
    obj.prepare_data()
    obj.setup(stage = 'fit')
